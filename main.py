...
def get_src_html(page_number):
    """Функция сохраняет все страницы во временную папку temp"""
...
def get_data_from_all_pages(header, count_pages):
    ...
    for page in range(int(count_pages)):
        ...
        for post in all_posts:
            ...
            # Получаю зарплату. Если блок есть - нормализую строку и записываю в словарь
            try:
            ...
            company = post.find('div', class_='vacancy-serp-item__meta-info-company').find('a').text.strip().replace(
                '\xa0', ' ')
            all_posts_tempdata['Компания'] = company
            all_posts_tempdata['Адрес'] = \
            post.find('div', class_='vacancy-serp-item__info').find_all('div', class_='bloko-text')[
                1].text.strip().replace('\xa0', ' ')
            all_posts_tempdata['Опыт'] = post.find('div',
                                                   class_='bloko-h-spacing-container bloko-h-spacing-container_base-0').find(
                'div', class_='bloko-text').text.strip()
...
def main():
    ...
    result = get_data_from_all_pages(HEADER, count_pages)
    # Текущее время
    ...
    print(f'Объявления успешно сохранены в файл result_{current_time}.json')
